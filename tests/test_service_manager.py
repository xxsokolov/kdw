...
from core.service_manager import ServiceManager, SERVICE_NAMES, INIT_D_PATH
...
async def test_get_all_statuses_error(service_manager):
    """Тест: ошибка при проверке статуса службы."""
    with patch('core.service_manager.run_command', new_callable=AsyncMock) as mock_run_command:
...
async def test_restart_service_success(service_manager):
    ...
    with patch('core.service_manager.run_command', new_callable=AsyncMock) as mock_run_command:
        ...
        mock_run_command.assert_called_once_with(f"{INIT_D_PATH}/{SERVICE_NAMES['shadowsocks']} restart")
